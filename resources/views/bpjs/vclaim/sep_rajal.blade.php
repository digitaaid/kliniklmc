...
@section('content')
                    ...
                    @isset($sep)
                        @foreach ($sep as $item)
                            <tr>
                                ...
                                <td>{{ $item->noSep }}</td>
                                <td>
                                    <x-adminlte-button theme="success" icon="fas fa-print" class="btn-xs"
                                        data-nosep="{{ $item->noSep }}" onclick="printSEP(this)" />
                                    <x-adminlte-button theme="warning" icon="fas fa-edit" class="btn-xs"
                                        data-nosep="{{ $item->noSep }}" />
                                    <x-adminlte-button theme="danger" icon="fas fa-trash" class="btn-xs"
                                        data-nosep="{{ $item->noSep }}" />
                                </td>
                                <td>{{ $item->tglSep }}</td>
    ...
    <x-adminlte-modal id="modalRujukan" title="Peserta Rujukan Peserta" theme="success" icon="fas fa-file-medical"
        size="xl">
        @php
    ...
    <x-adminlte-modal id="modalSuratKontrol" title="Surat Kontrol Peserta" theme="success" icon="fas fa-file-medical"
        size="xl">
        @php
            $heads = [
                'tglRencanaKontrol',
    ...
    <x-adminlte-modal id="modalPrintSEP" title="Print SEP" theme="success" icon="fas fa-file-medical" size="xl">
        <div id="iframeLoader" class="loader">
            <h4>Loading...</h4>
        </div>
        <iframe src="" id="iframePrintSEP" onload="iframeLoaded()" width="100%" height="500px"
...
@section('js')
    ...
    <script>
        ...
        function printSEP(button) {
            var nosep = $(button).data('nosep');
            var url = "{{ route('sep_print') }}?noSep=" + nosep;
            $('#iframePrintSEP').attr('src', url);
            $('#iframeLoader').show();
            $('#iframePrintSEP').hide();

            $('#modalPrintSEP').modal('show');
        }

        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <x-adminlte-textarea name="situation" label="Situation (S)" placeholder="Situation" fgroup-class="row"
                    ...
                    Salam dok, saya {{ Auth::user()->name }} dari keperawatan Klinik LMC izin melaporkan pasien dengan
                    data sbb:
                    Nama : {{ $antrian->nama }}
                    Umur :
                    {{ \Carbon\Carbon::parse($antrian->kunjungan->tgl_masuk)->diffInYears($antrian->kunjungan->tgl_lahir) }}
                    tahun, No RM : {{ $antrian->norm }}
                ...
                <x-adminlte-textarea name="assessment" label="Assessment (A)" placeholder="Assessment" rows="10"
                    fgroup-class="row" label-class="text-right col-3" igroup-size="sm" igroup-class="col-9"
                    enable-old-support required>
                    Detak Jantung : {{ $antrian->asesmenperawat->denyut_jantung ?? null }} spm , RR :
                    {{ $antrian->asesmenperawat->pernapasan ?? null }} spm
                    Tekanan Darah :
                    {{ $antrian->asesmenperawat->sistole ?? null }}/{{ $antrian->asesmenperawat->distole ?? null }}
                    mmHg , Suhu : {{ $antrian->asesmenperawat->suhu ?? null }} celcius
                    TT : {{ $antrian->asesmenperawat->tinggi_badan ?? null }} cm , BT :
                    {{ $antrian->asesmenperawat->berat_badan ?? null }} kg

    ...
    <div class="row">
        ...
        @if (isset($antrians))
            <div class="col-md-12">
                ...
                <x-adminlte-card title="Data Cetakan SEP" theme="warning" icon="fas fa-info-circle" collapsible>
                    <a href="{{ route('pdflaporansep') }}?tanggal={{ $request->tanggal }}" target="_blank"
                        rel="noopener noreferrer" class="btn btn-primary">Print PDF</a>
                    @php
                        $heads = ['No', 'Tanggal', 'Pasien', 'Kartu BPJS', 'No SEP', 'Status', 'PIC'];
                        $config['order'] = [1, 'asc'];
                        $config['paging'] = false;
